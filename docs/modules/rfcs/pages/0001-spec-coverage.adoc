...
and `asciidoc-html5`, and revises PLAN.md §9.2's data contract. Extended
by xref:0002-coverage-targets-beyond-asciidoc.adoc[RFC 0002], which
generalizes the measured page to non-AsciiDoc targets.
...
*Conflict precedence.* Claims and sidecar entries can target the same
block; resolution is deterministic:

* A claim on a `planned` block means the planned work shipped: the block
  is *verified*, and `bokfell coverage lint`
  (<<_cli_surface,§7>>) flags the now-stale `planned` entry for removal.
* A claim on a `non-normative` or `out-of-scope` block is a
  contradiction -- the test asserts a rule the sidecar says isn't there
  (or isn't ours) -- and is a hard `scan` error naming both sources; a
  human resolves it by fixing whichever is wrong.
* A block targeted by more than one sidecar list (e.g. both
  `non-normative` and `planned`) is a hard error when the sidecar loads.
...
* *Path* is as written in the scanned repo (matching today's
  `track_file!` convention), and resolution is *scoped*, not global:
  a claim resolves by suffix-matching against catalog pages sourced
  from the same `coverage.scan` entry's repo as the test root -- tests
  claim their own repo's spec pages by default, so identical paths in
  other components never collide. A cross-repo claim names the page
  with a full Antora-style resource ID
  (`component:module:page.adoc`) instead of a path. Within that scope
  the path identifies a *page*, not a version: the claim is resolved
  against each measured version of the page independently, and the
  excerpt decides per version -- versions whose block text matches are
  verified; versions whose text has since diverged are simply
  unmatched, which is not an error. Only a path matching no page in
  scope, an excerpt matching *no* measured version, or an excerpt
  ambiguous *within one version's page* is a hard error.
* *Excerpt* is matched against the block's *source* text after
  whitespace normalization (the same collapse rules the search indexer
  uses) -- normalization collapses whitespace only, and inline markup is
  not stripped, so excerpts are quoted from the `.adoc` source and stay
  reviewable against it. The excerpt may be any contiguous substring of
  one block. Matching is *exact* after normalization -- fuzziness would
  silently heal spec drift, and drift detection is a feature we keep on
  purpose: when the upstream paragraph changes, exactly the claims that
  quote it fail to resolve, and no others.
...
* Claims are *block-granular and binary*: there is no "partially
  verifies" qualifier, and a claim on a paragraph that states two rules
  is accepted rounding. Revisit if migrated or new pages produce
  evidence that the rounding hides real gaps.
...
`reviewed = true` at the top. `reviewed` is *per page*; a finer
per-section scope can be added later if long pages need incremental
classification. This keeps the pressure state meaningful: a page
becomes "fully classified" by an explicit human act, not by the
heuristic's silence.
...
* `bokfell coverage scan` -- aggregate (reusing `bokfell-aggregate`), parse
  ...
  write the coverage database. The database is *ephemeral* -- recomputed
  by every scan, never committed; `--format codecov` is the only
  persisted export. Unresolved or ambiguous excerpts are errors listed
  with file/line provenance.
* `bokfell coverage report [--format table|json|codecov]` -- rollups.
  `--format codecov` projects block states onto line ranges via the
  source map and emits the interim tools' schema, so existing CI
  uploads swap over without pipeline changes. The projection rule is
  explicit: every source line of a `verified` block emits `1`; every
  source line of a `planned`, `uncovered`, or `unclassified` block
  emits `0`; lines of `out-of-scope` and `non-normative` blocks, and
  lines belonging to no block (blank lines, delimiters), are omitted --
  so hit/miss/absent corresponds exactly to
  in-denominator-covered / in-denominator-uncovered /
  out-of-denominator, and the migration parity check
  (<<_migration,§9>>) compares at this projection level.
...
** _Serve mode:_ reuse the PLAN.md §9.3 edit round-trip verbatim -- "open
   ...
   editor at the test function. The affordance follows the edit loop's
   locality rule: it is offered only when the claim's rust file resolves
   inside a *local* source (a worktree, or a repo mapped with
   `--local url=path`), whose resolved absolute path joins the editable
   allowlist. Claims whose repo is cache-backed or remote-only render
   the build-mode URL link instead -- a cached clone is never exposed to
   the editor.
...
All four questions this RFC originally left open were resolved
2026-09-13; the decisions are folded into the sections above and
recorded here.

. *Excerpt normalization* -- whitespace collapse only. Excerpts match
  the parsed block's source text with inline markup intact, so claims
  are quoted from and reviewable against the `.adoc` file
  (<<_claims_the_verifies_marker,§4>>).
. *Sub-block granularity* -- claims are block-granular and binary; a
  paragraph stating two rules is accepted rounding. An initial call, to
  be revisited if migration or new pages show the rounding hiding real
  gaps (<<_claims_the_verifies_marker,§4>>).
. *`reviewed = true` semantics* -- per page to start; a per-section
  scope can be added later if long pages need incremental
  classification (<<_classification_the_coverage_map,§5>>).
. *Coverage database* -- ephemeral, recomputed by every `scan` and
  never committed; `--format codecov` is the only persisted export
  (<<_cli_surface,§7>>).
